...
-- Agregue la columna nivel_atencion VARCHAR(20) DEFAULT NULL a la tabla consultas. 
alter table consultas add column nivel_atencion VARCHAR(20) DEFAULT NULL;


-- Cree la funcion fn_nivel_atencion(p_costo DECIMAL(10,2)) que retorne: BASICO si costo <= 100000, INTERMEDIO si entre 100001 y 200000, ESPECIALIZADO si > 200000. 
  
  delimiter //
  create function fn_nivel_atencion(c_costo DECIMAL(10,2))
  returns varchar (50)
  deterministic
  begin
  
    declare costo_actual int;
    select costo into costo_actual from consultas where consulta_id  = c_consulta_id ;
    if  costo_actual <= 100000 then return 'BASICO';
    elseif  costo_actual >= 100001 <=200000 then return 'INTERMEDIO';
    elseif  costo_actual > 200000 then return 'ESPECIALIZADO';
    else return 'NO_REGISTRA';
    end if;
...
select consulta_id, diagnostico, costo,estado, fn_nivel_atencion(c_costo) as nivel_atencion
from consultas
order by nivel_atencion desc;

-- Luego actualice la columna nivel_atencion de todas las consultas usando la funcion en un UPDATE. 

-- update consultas set nivel_atencion 
 
 
-- Finalmente, escriba una consulta que muestre cuantas consultas hay por nivel_atencion (excluyendo las canceladas), ordenado de mayor a menor.

-- Clausulas requeridas: ALTER TABLE ADD COLUMN, CREATE FUNCTION con CASE, UPDATE SET col=funcion(), SELECT GROUP BY con WHERE y ORDER BY
